...
Claude Code で作業
      ...
      │    └─ flush  [SessionEnd] ────→ References/ スタブ (L3)
      │
...
| L3 | ≥ 9 | `References/` スタブ + Daily チェックリスト |
...
1. `jq` と `python3` の確認
2. `SECOND_BRAIN_VAULT_PATH` を `.claude/settings.local.json` に書き込む
3. `CLAUDE.md` の vault パスを自動パッチ（Claude Code が正しい vault を参照するよう）
4. スクリプトに `chmod +x`
5. pre-commit フックのシンリンクを設置（`.git/hooks/pre-commit → hooks/pre-commit`）
...
```
Ideas/           — 自動スケッチ、reviewed_status: false（低スコア・未レビュー）
